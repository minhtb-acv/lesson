...
define("LESSON_PAGE_DDMATCH", "11");

class lesson_page_type_ddmatch extends lesson_page {

    protected $type = lesson_page::TYPE_QUESTION;
    protected $typeid = LESSON_PAGE_DDMATCH;
    protected $typeidstring = 'ddmatch';
    ...
    public function get_typestring() {
        if ($this->string === null) {
            $this->string = get_string($this->typeidstring, 'local_lesson');
        }
        return $this->string;
    }
    ...
    protected function make_answer_form($attempt = null) {
        ...
        $params = array('answers' => $answers, 'useranswers' => $useranswers, 'lessonid' => $this->lesson->id, 'contents' => $this->get_contents());
        $mform = new lesson_display_answer_form_ddmatch($action, $params);
        return $mform;
    }
...
//                $answer->response = htmlspecialchars($properties->response_editor[$i]['text']);
            ...
            if (isset($answer->answer) && $answer->answer != '') {
                ...
                $this->save_answers_files($context, $PAGE->course->maxbytes, $answer, $properties->answer_editor[$i]);
...
class lesson_add_page_form_ddmatch extends lesson_add_page_form_base {

    public $qtype = 'ddmatch';
    public $qtypestring = 'ddmatch';
...
//        $numbernewanswers = 3;
//        
//        if ($this->_customdata['xuananh']) {
//            $numbernewanswers += 1;            
//        }
//        $this->_form->addElement('html', '<input type="hidden" name="numbernewanswers" value="'.$numbernewanswers.'">');

        $this->_form->addElement('header', 'correctresponse', get_string('correctresponse', 'lesson'));
        $this->_form->addElement('editor', 'answer_editor[0]', get_string('correctresponse', 'lesson'), array('rows' => '4', 'columns' => '80'), array('noclean' => true, 'maxfiles' => EDITOR_UNLIMITED_FILES, 'maxbytes' => $this->_customdata['maxbytes']));
        ...
        $this->_form->addElement('editor', 'answer_editor[1]', get_string('wrongresponse', 'lesson'), array('rows' => '4', 'columns' => '80'), array('noclean' => true, 'maxfiles' => EDITOR_UNLIMITED_FILES, 'maxbytes' => $this->_customdata['maxbytes']));
        ...
        for ($i = 2; $i < $this->_customdata['lesson']->maxanswers + 2; $i++) {
            ...
            $count = $i;
//            $this->_form->addElement('editor', 'response_editor[' . $count . ']', $label, array('rows' => 3));
...
//        $this->_form->addElement('html', '<input name="addnewanswers" value="Blanks for 3 more questions" type="submit" onclick="skipClientValidation = true;" id="id_addnewanswers">');
    }

}

class lesson_display_answer_form_ddmatch extends moodleform {
    ...
    public function definition() {
         ...
         */
        $PAGE->requires->css('/local/lesson/css/ddmatch.css');
        $jsmodule = array(
            'name' => 'local_lesson',
            'fullpath' => '/local/lesson/dragdrop.js',
            'requires' => array('dd-delegate', 'dd-drop-plugin', 'dd-proxy', 'dd-constrain', 'selector-css3'));
        // -- end 
        // Disable shortforms.
         ...
         * List Lesson Answer
         */
        $mform->addElement('html', '<div id="play" class="lesson-dd">');
        $mform->addElement('html', '<div id="dragdrop-lesson"><ul>');
        $answersResponse = $answers;
        shuffle($answersResponse);
        foreach ($answersResponse as $answer) {

            if ($answer->response != null) {

                $mform->addElement('html', '<li data-id="' . $answer->id . '" data-value="' . htmlspecialchars(trim($answer->response)) . '">');
                $mform->addElement('html', htmlspecialchars_decode($answer->response));
        ...
        /**
         * List box where you can drop
         */
        $mform->addElement('html', '<table class="drop-lesson-answer">');
        foreach ($answers as $answer) {

            $answersParams[] = $answer->id;
            ...
            if ($answer->response != null) {
                ...
                $context = context_module::instance($PAGE->cm->id);
                $answer->answer = file_rewrite_pluginfile_urls($answer->answer, 'pluginfile.php', $context->id, 'mod_lesson', 'page_answers', $answer->id);
                $mform->addElement('html', '<tr class="answer-item" id="item-' . $answer->id . '">');
                $mform->addElement('html', '<td>' . format_text($answer->answer, $answer->answerformat, $options) . '</td>');
                $mform->addElement('html', '<td><div id="drop-' . $answer->id . '" class="lesson-drop"><span class="draghere">' . get_string("draganswerhere", "local_lesson") . '</span></div></td>');
                $mform->addElement('hidden', $responseid, 0);
        ...
        //-- end table lesson-answer
        $mform->addElement('html', '</div>');
        //-- end div class lesson-dd

        $PAGE->requires->js_init_call('M.local_lesson.init', array(implode(',', $answersParams)), false, $jsmodule);
