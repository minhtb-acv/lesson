...
class mod_lesson_renderer extends plugin_renderer_base {
    ...
    public function add_page_links(lesson $lesson, $prevpageid=false) {
        ...
        //Anh Le Xuan (AnhLX) Ver2 Add
        global $DB;
        $addQuestionBankLink = new moodle_url('/local/lesson/lesson.php',array('id'=>$this->page->cm->id, 'pageid'=>$prevpageid));
        $links[] = html_writer::link($addQuestionBankLink, get_string('addfromquestionbank', 'local_lesson'));
    ...
    public function add_first_page_links(lesson $lesson) {
        ...
        //Anh Le Xuan (AnhLX) Ver2 Add
        global $DB;
        $cm = get_coursemodule_from_id('lesson', $this->page->cm->id, 0, false, MUST_EXIST);
    ...
    public function page_action_links(lesson_page $page, $printmove, $printaddpage=false) {
        ...
        if ($printaddpage) {
             ...
             * Add link
             */
            $options[1000] = get_string('addfromquestionbank', 'local_lesson');
