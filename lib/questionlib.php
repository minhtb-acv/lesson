...
function question_pluginfile($course, $context, $component, $filearea, $args, $forcedownload, array $options=array()) {
    ...
    if ($filearea === 'export') {
        ...
        /**
         * Anh Le Xuan Ver2 Add
         */
        $saveXML = optional_param('saveXML', 0, PARAM_INT); //Anh Le Xuan Add
        if ($saveXML == 1) {
            file_put_contents($CFG->dataroot.'/temp/'.$filename, $content);

            $importDataLesson = array(
                'urlImport' => $CFG->wwwroot.'/local/lesson/import_to_lesson.php',
                'file' =>  $CFG->dataroot.'/temp/'.$filename,
                'status' => 1
            );

            echo json_encode($importDataLesson);
            die();
        } else {
