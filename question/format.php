...
class qformat_default {
    ...
    public function exportprocess() {
         ...
         * Anh Le Xuan Ver2
         */
        $lessonPopup = optional_param('lessonPopup', 0, PARAM_INT);
        $saveXML = optional_param('saveXML', 0, PARAM_INT);
        $questionsCourse = array();
        //end
        ...
        foreach ($questions as $question) {
            ...
            /**
             * Anh Le Xuan Ver2
             * Get ID and Name Question send to popup
             */
            if ($lessonPopup == 1) {
                $questionsCourse[] = array(
            ...
            /**
             * Save with some selected question
             * more : local/lesson/lesson.js
             */
            if ($saveXML == 0) {
                ...
                $qlid = optional_param('qlid', 0, PARAM_TEXT);
                $selectQuestion = (explode(",", $qlid));
        ...
        if ($count==0) {
            if ($lessonPopup == 1) {
                echo json_encode(array(
                    'error' => 1,
                    'data' => get_string('noquestions', 'question')
                ));
                die;
            } else {
        ...
        if ($lessonPopup == 1) {
            echo json_encode($questionsCourse);
            die();
        }
        // End
